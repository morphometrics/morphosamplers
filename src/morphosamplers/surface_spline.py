...
from .utils import minimize_point_strips_pair_distance, extrapolate_point_strip
...
class _SplineSurface(EventedModel):
    ...
    def _generate_meta_splines(self):
        ...
        # extrapolate where nans are present
        extended = [
            extrapolate_point_strip(s, d, self.separation)
            for s, d in zip(aligned, equidistant_directions)
        ]

        meta_spline_control_points = np.stack(extended, axis=1)
    ...
    def _generate_meta_meta_splines(self):
        us = [spline._get_approximate_equidistance_u(self.separation) for spline in self._meta_splines]
        ...
        for p in np.stack(equidistant_points, axis=1):
            tck, u, = splprep(p.T, s=0, k=self.order)
            tcks.append(tck)
            us.append(u)
...
class SplineSurfaceGrid(_SplineSurface):
    ...
    def sample_surface_orientations(self, inside_point=None):
        ...
        equidistant_y_vecs = [
            np.stack(splev(u, tck, der=1), axis=1)
            for u, tck in zip(self._meta_meta_meta_splines_u, self._meta_meta_meta_splines_tck)
        ]
